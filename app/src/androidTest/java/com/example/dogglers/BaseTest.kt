...
open class BaseTest {
    ...
    val lastPosition = DataSource.LAPTOPS.size - 1


    private fun hasListItemContent(name: String, brand: String, price: String, imageResource: Int) {
        onView(withText(name))
            .check(matches(isDisplayed()))
        onView(withText(brand))
            .check(matches(isDisplayed()))
        onView(withText(price))
            .check(matches(isDisplayed()))
    ...
    fun checkFirstPosition() {
        hasListItemContent("ROG GL552VW", "Brand: Asus", "11.000.000: ",
            R.drawable.tzeitel)
    }
